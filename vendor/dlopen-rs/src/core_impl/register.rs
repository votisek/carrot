...
pub(crate) struct GlobalDylib {
    ...
    /// Thread marker of the dlopen session currently relocating this entry.
    /// Lets a recursive dlopen from an initializer recognize its own session
    /// instead of waiting on itself. Zero when no session owns the entry.
    pub(crate) relocating_owner: usize,
}
