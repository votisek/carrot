...
/// the host's static-tls surplus: (size, align) -> a tp-relative offset
/// valid in every thread, pointing at zeroed memory
pub type StaticTlsAlloc = fn(usize, usize) -> Option<isize>;
static STATIC_TLS_ALLOC: core::sync::atomic::AtomicUsize =
    core::sync::atomic::AtomicUsize::new(0);

/// register the allocator behind DF_STATIC_TLS libraries; without one,
/// loading them fails as unsupported
pub fn set_static_tls_allocator(f: StaticTlsAlloc) {
    STATIC_TLS_ALLOC.store(f as usize, Ordering::SeqCst);
}
...
impl ThreadDtv {
    ...
    fn get_or_allocate(&mut self, mod_id: usize) -> Option<*mut u8> {
        ...
        // Static modules live at a fixed tp offset, never on the heap
        if let Some(offset) = template.tp_offset {
            let ptr = unsafe { crate::arch::get_thread_pointer().offset(offset) };
            self.dtv[mod_id] = Some(DtvEntry::Static { ptr });
            return Some(ptr);
        }
...
impl TlsResolver for DefaultTlsResolver {
    ...
    fn register_static(tls_info: &TlsInfo) -> Result<(usize, isize)> {
        // the surplus is pre-zeroed per thread; an init image would need
        // copying into every already-running thread, which nothing we
        // load requires (driver tls shims are pure tbss)
        if tls_info.filesz != 0 {
            return Err(tls_error("static tls with an init image is unsupported"));
        }
        let f = STATIC_TLS_ALLOC.load(Ordering::SeqCst);
        if f == 0 {
            return Err(tls_error("unsupport static tls"));
        }
        let f: StaticTlsAlloc = unsafe { core::mem::transmute(f) };
        let offset = f(tls_info.memsz, tls_info.align)
            .ok_or_else(|| tls_error("static tls surplus exhausted"))?;
